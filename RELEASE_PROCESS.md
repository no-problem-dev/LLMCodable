...
LLMCodableの新バージョンをリリースする手順を説明します。
...
```bash
# リリースブランチに切り替え（例: release/v1.0.1）
...
**重要**: バージョン番号は必ずブランチ名と一致させる（`release/v1.0.1` → `[1.0.1]`）
...
**このマージで自動的に実行されること:**
1. タグ `v1.0.1` が作成される
2. GitHub Release `v1.0.1` が作成される
3. 次のリリースブランチ `release/v1.0.2` が作成される
...
| 変更内容 | バージョン | 例 |
|---------|-----------|-----|
| バグ修正のみ | PATCH | 1.0.1 → 1.0.2 |
| 新機能追加（後方互換） | MINOR | 1.0.2 → 1.1.0 |
...
### 追加
- **新しいエンコード形式**: YAML形式のエンコードをサポート

### 修正
- **デコードエラー**: 日本語テキストのデコード時に発生するエラーを修正
```
